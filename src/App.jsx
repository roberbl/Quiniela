...
export default function App() {
  ...
  async function startSession(profile, sessionUser, persistProfile = true) {
    ...
    try {
      ...
      setMessage(`Has entrado, pero no se pudo guardar el perfil${code}. Revisa Firestore Database y publica las reglas.`);
    }
  }
  ...
  async function saveAllResults() {
    ...
    await Promise.all(matches.map((match) => upsertMatch(round.id, match)));
    recalc();
    setMessage('Resultados guardados y clasificaciones actualizadas');
  }
  function recalc() {
